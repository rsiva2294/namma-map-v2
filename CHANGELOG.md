...
- **Backend Summary Caching**: Deployed a dedicated `/state` endpoint with an isolated 60-second in-memory cache to prevent ECI rate limits during high traffic.
- **Manual Dashboard Refresh**: Added a refresh capability to the Election Dashboard to allow users to update state tallies on demand.

### Changed
- **Dashboard Optimization**: Refactored `ElectionDashboard` to use the unified state summary endpoint rather than computing totals client-side from individual cached constituencies.
- **Winner Highlighting**: Enhanced the UI of the `ConstituencyDetailCard` to distinctly highlight winning candidates with green background accents and bold green text.
