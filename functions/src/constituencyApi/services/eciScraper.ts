import { parseEciHtml, parseStateSummaryHtml, StateSummary } from '../parser/htmlParser';
...
let stateSummaryCache: { data: StateSummary[]; expiresAt: number } | null = null;
const STATE_SUMMARY_CACHE_TTL_MS = 60 * 1000; // 60 seconds

export const fetchStateSummary = async (): Promise<StateSummary[]> => {
  const now = Date.now();
  ...
  const html = await response.text();
  const data = parseStateSummaryHtml(html);
